...
//Camera
Camera camera(glm::vec3(0.0f, 0.0f, 3.0f)); //posicion inicial de la camara
...
void LoadTextures()
{
	...
	/*t_muertos = generateTextures("Texturas/Muertos.jpg", 0);*/ //no contiene canal alpha y se pone valor de 0
	t_muertos = generateTextures("Texturas/Muertos.png", 1);//Con transparencia
	t_dado = generateTextures("Texturas/Cube03.jpg", 0);
}
...
void myData()
{
	float vertices[] = {
		...
		// 0.5f,  0.5f, 0.0f,   0.25f, 1.0f, // top right
		// 0.5f, -0.5f, 0.0f,  0.25f, 0.8f, // bottom right
		//-0.5f, -0.5f, 0.0f,   0.0f, 0.8f, // bottom left
		//-0.5f,  0.5f, 0.0f,   0.0f, 1.0f  // top left 

		////Repetir Textura
		// 0.5f,  0.5f, 0.0f,   3.0f, 2.0f, // top right
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
		...
		glBindTexture(GL_TEXTURE_2D, t_muertos);
		//glDrawArrays(GL_TRIANGLES, 0, 36); //A lonely cube :(
		glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0); //dibujo cuadrado

		/***   Segundo objeto  **/
		
		glBindVertexArray(VAO[1]);
		modelOp = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, -4.0f));
		myShader.setMat4("model", modelOp);
		myShader.setVec3("aColor", 1.0f, 1.0f, 1.0f);
		glBindTexture(GL_TEXTURE_2D, t_dado);
		glDrawArrays(GL_TRIANGLES, 0, 36); //A lonely cube :(
